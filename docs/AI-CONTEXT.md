...
- **Workers Builds**: モノレポでは「Root directory」でサブディレクトリを指定し、「Build Watch Paths」の除外パスで対象外ディレクトリを指定できる。Deploy Hooksにも対応済み(2026年4月〜)。Free枠はビルド3,000分/月・同時ビルド1・ビルドタイムアウト20分・ビルド時変数64個。**push起因の自動ビルドを無効化する設定はダッシュボードに存在しない**(2026-09、公式ドキュメントで確認済み。プロダクションブランチへのpushは常にビルドを起動する)。本プロジェクトはmainへのpushでも生成処理を含むビルドが走ることを許容し(個人プロジェクトのためpush頻度は低い)、`docs/**`・`.superpowers/**` のみBuild Watch Pathsの除外パスで対象外にしている。
